...
const filters = [
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Early Stage", value: "Early Stage", name: "Funding Stage" },
      { label: "Growth Stage", value: "Growth Stage", name: "Funding Stage" },
  ...
  const handleCheckboxChange = (filterName: string, value: string) => {
    setSelectedCheckboxes((prevState) => {
      ...
      if (isSelected) {
        return {
          ...prevState,
          [filterName]: prevState[filterName].filter((item) => item !== value),
        };
      } else {
        return {
          ...prevState,
      ...
      {filters.map((filter) => (
        <div className={styles.filtersWrapper} key={filter.title}>
          ...
          {filter.options.map((option) => (
            <label className={styles.radioContainer} key={option.value}>
              <input
                type={filter.type}
                name={option.name}
                value={option.value}
                checked={
                  filter.type === "radio"
                    ...
                    : selectedCheckboxes[filter.title]?.includes(option.value)
                }
                onChange={() => {
